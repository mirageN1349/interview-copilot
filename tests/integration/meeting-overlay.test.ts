...
describe('meeting overlay behavior', () => {
  ...
  it('opens as an interactive, resizable panel with usable dimensions', () => {
    ...
    expect(shell).not.toContain('name="overlay-content"')
    expect(shell).not.toContain('mode="out-in"')
    expect(shell).toContain('key="live"')
    expect(shell).toContain('key="status"')
    expect(shell).toContain('class="overlay-content-view"')
    expect(shell).toContain('animation: overlay-content-enter 210ms cubic-bezier(0.22, 1, 0.36, 1)')
    expect(shell).toContain('@keyframes overlay-content-enter')
